...
'9:16': 'aspect-[9/16]'
  ...
  return (
    <>
      <SoftBackdrop/>
      <div className="mt-32 min-h-screen px-6 md:px-16 1g:px-24 x1:px-32">
        <div className='mb-8'>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {
                Array.from({length:6}).map((_,i)=>(
                  <div key={i} className='rounded-2x1 bg-white/6 border border-white/10 animate-pulse h-[260px]'/>
                ))
              }
            </div>
          )
        ...
        {/* empty state */}
        {
          !loading && thumbnails.length === 0 &&(
            <div className='text-center py-24'>
              ...
              <p className="text-sm text-zinc-400 mt-2">Generate your first thumbnail to see more </p>
            </div>
          ) 
        }
        {/* grid */}
        {
          !loading && thumbnails.length > 0 && (
            <div className="columns-1 sm:columns-2 1g:columns-3 2xl:columns-4 gap-8">
                {
                    ...
                    return (
                      <div key={thumb._id} onClick={()=>navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                        <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                          {
                            thumb.image_url?(<img src={thumb.image_url} className='w-full h-full object-cover group-hover:scale-105 transition-transform duration-300'/>):(
