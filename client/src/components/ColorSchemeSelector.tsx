        ...
        <div className='grid grid-cols-6 gap-3'>
            {
                colorSchemes.map((scheme)=>(
                    <button type="button" key={scheme.id} onClick={()=>onChange(scheme.id)}
                    title={scheme.name} className={`relative rounded-lg transition-all ${value === scheme.id && 'ring-2 ring-pink-500'}`}>
